...
describe('API Endpoints', () => {
  ...
  describe('Auth Routes', () => {
    it('POST /api/auth/register', async () => {
      ...
      User.prototype.save = jest.fn().mockResolvedValue({ _id: 'user123', email: 'test@test.com' });
    ...
    it('POST /api/auth/login', async () => {
      User.findOne.mockResolvedValue({ _id: 'user123', password: 'hashed_pw', email: 'test@test.com' });
  ...
  describe('User Routes', () => {
    ...
    it('PUT /api/user/profile', async () => {
      ...
      User.findById.mockResolvedValue({ _id: 'user123', name: 'Old', save: mockSave });

      const res = await request(app).put('/api/user/profile').send({ name: 'New' });
      expect(res.statusCode).toBe(200);
    ...
    it('PUT /api/user/profile/avatar (Image Processing logic check)', async () => {
      ...
      const res = await request(app).put('/api/user/profile/avatar').send({ avatar: 'base64_string_here' });
      expect(res.statusCode).toBe(200);
    ...
    it('POST /api/user/favorites/:recipeId', async () => {
      const mockSave = jest.fn().mockResolvedValue();
      User.findById.mockResolvedValue({ _id: 'user123', favorites: [], save: mockSave });
    ...
    it('GET /api/user/favorites', async () => {
      User.findById.mockReturnValue({
        populate: jest.fn().mockResolvedValue({ _id: 'user123', favorites: [{ _id: 'rec1' }] })
      });
  ...
  describe('Recipe Routes', () => {
    it('POST /api/recipes', async () => {
      ...
      const res = await request(app).post('/api/recipes').send({ title: 'New Recipe' });
      expect(res.statusCode).toBe(201);
    });
    ...
    it('PUT /api/recipes/:id', async () => {
      Recipe.findById.mockResolvedValue({ _id: 'rec1', user: 'user123', isUserCreated: true });
      Recipe.findByIdAndUpdate.mockResolvedValue({ _id: 'rec1', title: 'Updated' });

      const res = await request(app).put('/api/recipes/rec1').send({ title: 'Updated' });
      expect(res.statusCode).toBe(200);
    });

    it('POST /api/recipes/:id/rate', async () => {
      const mockSave = jest.fn().mockResolvedValue();
      Recipe.findById.mockResolvedValue({ _id: 'rec1', ratings: [], save: mockSave });

      const res = await request(app).post('/api/recipes/rec1/rate').send({ rating: 4 });
      expect(res.statusCode).toBe(200);
    });
  ...
  describe('MealPlan Routes', () => {
    ...
    it('POST /api/mealplan', async () => {
      ...
      const res = await request(app).post('/api/mealplan').send({ startDate: '2023-01-01', week: {} });
      expect(res.statusCode).toBe(201);
    });

    it('GET /api/mealplan', async () => {
      MealPlan.findOne.mockReturnValue({
        populate: jest.fn().mockResolvedValue({ _id: 'mp1', week: createEmptyWeek() })
      });
    ...
    it('GET /api/mealplan/shopping-list', async () => {
      ...
      const res = await request(app).get('/api/mealplan/shopping-list?startDate=2023-01-01');
      expect(res.statusCode).toBe(200);
    });

    it('PUT /api/mealplan/:day/:slot', async () => {
      MealPlan.findOne.mockResolvedValue({ _id: 'mp1', week: createEmptyWeek(), save: jest.fn().mockResolvedValue(), markModified: jest.fn() });

      const res = await request(app).put('/api/mealplan/monday/Breakfast').send({ startDate: '2023-01-01', recipeId: 'rec1' });
      expect(res.statusCode).toBe(200);
    });
    ...
    it('DELETE /api/mealplan/:day/:slot', async () => {
      ...
      MealPlan.findOne.mockResolvedValue({ _id: 'mp1', week: weekWithRec, save: jest.fn().mockResolvedValue() });

      const res = await request(app).delete('/api/mealplan/monday/Breakfast?startDate=2023-01-01');
      expect(res.statusCode).toBe(200);
    });
    ...
    it('PUT /api/mealplan', async () => {
      ...
      const res = await request(app).put('/api/mealplan').send({ startDate: '2023-01-01', week: {} });
      expect(res.statusCode).toBe(200);
    });
  });
  ...
  describe('Meal Routes', () => {
    it('POST /api/meals', async () => {
      ...
      const res = await request(app).post('/api/meals').send({ name: 'New Meal' });
      expect(res.statusCode).toBe(201);
    });
    ...
    it('PUT /api/meals/:id', async () => {
      ...
      const res = await request(app).put('/api/meals/meal1').send({ name: 'Updated Meal' });
      expect(res.statusCode).toBe(200);
    });
